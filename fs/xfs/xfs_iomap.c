	...
	struct iomap		*iomap)
{
	 ...
	 * mapping code below.
	 * 以指定操作所需的方式锁定inode,并检查尽可能多的会导致阻塞的条件
	 * 下面的映射代码中删除了大量的非阻塞检查
	 */
	 ...
	 * 如果有必要，可以中断共享范围
	 * 非阻塞IO的检查已经预先完成，所以我们不需要在这里进行
	 */
	if (xfs_is_reflink_inode(ip)) {
		/* if zeroing doesn't need COW allocation, then we are done.
		 * 如果归零不需要COW分配，那么我们就完成了
		 * */
		if ((flags & IOMAP_ZERO) &&
	 ...
	 * 我们将映射的最大长度限制为合理的大小，以保持完成的工作块与工作写回对称。
	 * 这是一个完全随机的数字，作为初始测试的最佳猜测。
	 *
	 * Note that the values needs to be less than 32-bits wide until the
	 * lower level functions are updated.
	 * 请注意，在更新较低级别的函数之前，这些值需要小于 32 位宽。
	 */
	 ...
	 * return.
	 * xfs_iomap_write_direct() 需要共享锁。它在返回时解锁
	 */
	// 如果是排他锁，那么需要进行锁的降级
